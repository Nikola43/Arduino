...
void setup()
{
    pinMode(BACK_IZQ, OUTPUT);
    pinMode(FRONT_IZQ, OUTPUT);
    pinMode(BACK_DER, OUTPUT);
    pinMode(FRONT_DER, OUTPUT);
  Serial.begin(115200);
  Wire.begin();
...
void turnOn(double AcX, double AcY, double AcZ, double GyX, double GyY, double GyZ)
{
  ...
  for(x=0; x<200; x++)
  {
               analogWrite(BACK_IZQ,  x);
      analogWrite(FRONT_IZQ, x);
      analogWrite(BACK_DER,  x);
      analogWrite(FRONT_DER, x);
      //fly(AcX, AcY, AcZ, GyX, GyY, GyZ);
      delay(1);
      Serial.println(x);
  }
...
void fly(double AcX, double AcY, double AcZ, double GyX, double GyY, double GyZ)
{
  ...
  if (AcX < (-800))
  {
      analogWrite(FRONT_IZQ,  200);
      analogWrite(FRONT_DER, 200);
  }
  else
  {
  ...
  if (AcX > (-500))
  {
    analogWrite(BACK_IZQ, 200);
    analogWrite(BACK_DER, 200);
  }
  else
  {
